...
function DiveLogDetail(props){
  return (
    <React.Fragment>
      ...
      <p><em>Time under water for second dive:{diveLog.time2}</em></p>
      <p><em>Depth of third dive:{diveLog.depth3}</em></p>
      <p><em>Time under water for third dive:{diveLog.time3}</em></p>
      <p><em>Depth of fourth dive:{diveLog.depth4}</em></p>
      <p><em>Time under water for fourth dive:{diveLog.time4}</em></p>
      ...
      <Button variant="danger" onClick={() => props.onClickingDelete(diveLog.id) }>Delete Dive Log</Button>
      <hr/>
    </React.Fragment>
  );
}
